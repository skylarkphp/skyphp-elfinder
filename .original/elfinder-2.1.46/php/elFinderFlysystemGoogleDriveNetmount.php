...
class elFinderVolumeFlysystemGoogleDriveNetmount extends ExtDriver
{
    ...
    public function netmountPrepare($options)
    {
        ...
        try {
            ...
            if ($aToken) {
                try {
                    ...
                    if ($options['user'] !== 'init') {
                        $this->session->set('GoogleDriveAuthParams', $options);
                        return array('exit' => true, 'error' => [elFinder::ERROR_REAUTH_REQUIRE,$e->getMessage()]);
                    }
                }
            ...
            if ($options['user'] === 'init') {
                ...
                if (!$aToken && empty($_GET['code'])) {
                    ...
                    ////return array('exit' => true, 'code' => $_GET['code']);
                    if (!empty($_GET['code'])) {
        ...
        ///if (!$aToken) {
        ///    return array('exit' => true, 'error' => [elFinder::ERROR_REAUTH_REQUIRE,$_GET['code']]);
        ///}
